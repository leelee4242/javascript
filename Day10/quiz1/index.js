...
const makeProfile = (x) => ` 
    <div class="profile">
        <img src="${x.image}" alt="" />
        <div class="info">
          <span class="name">${x.lastName + " " + x.maidenName + " " + x.firstName}</span>
          <span class="birth">${x.birthDate.replaceAll("-", "/")}</span>
          <span class="department">${x.company.department}</span>
...
data.users.forEach((x) => {
  section.insertAdjacentHTML("beforeend", makeProfile(x));
});